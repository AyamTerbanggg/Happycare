...
<body>
    ...
    <div class="main-content">
        <div class="page-title">Edit Tourism Spot: {{ $tourism->name }}</div>
        <div class="form-card">
            <form action="{{ route('admin.tourism.update', $tourism->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="city" id="city" value="{{ old('city', $tourism->city) }}" required>
                </div>
                <div class="form-group">
                    <label for="address">Address</label>
                    <input type="text" name="address" id="address" value="{{ old('address', $tourism->address) }}" required>
                </div>
                <div class="form-group">
                    <label for="description">Description</label>
                    <textarea name="description" id="description" rows="5" required>{{ old('description', $tourism->description) }}</textarea>
                </div>
                <div class="form-group">
                    <label for="price">Price</label>
                    <input type="text" name="price" id="price" value="{{ old('price', $tourism->price) }}" required>
                </div>
                <div class="form-group">
                    <label>Current Image</label>
                    @if ($tourism->image)
                        <img src="{{ asset('storage/' . $tourism->image) }}" alt="{{ $tourism->name }}" class="img-preview">
                    @else
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="facilities" id="facilities" value="{{ old('facilities', is_array($tourism->facilities) ? implode(',', $tourism->facilities) : $tourism->facilities) }}">
                </div>
                <div class="form-group">
                    <label for="maps_url">Google Maps Link</label>
                    <input type="url" name="maps_url" id="maps_url" value="{{ old('maps_url', $tourism->maps_url) }}">
                </div>
                <div class="form-actions">
                    <button type="submit" class="submit-btn"><i class="fas fa-save"></i><span>Update Tourism Spot</span></button>
                    <a href="{{ route('admin.tourism.index') }}" class="cancel-btn"><i class="fas fa-arrow-left"></i><span>Cancel</span></a>
                </div>
            </form>
        </div>
    </div>
